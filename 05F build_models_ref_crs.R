...
ref.train.crs<-t(apply(as.matrix(ref.train),1,function(x) continuumRemoval(x,wav=400:2400,method="substraction")))
ref.train.crs<-spectra(ref.train.crs,bands=bands(ref.train),names=names(ref.train))
meta(ref.train.crs)<-meta(ref.train)

ref.test.crs<-t(apply(as.matrix(ref.test),1,function(x) continuumRemoval(x,wav=400:2400,method="substraction")))
ref.test.crs<-spectra(ref.test.crs,bands=bands(ref.test),names=names(ref.test))
meta(ref.test.crs)<-meta(ref.test)
...
solubles_mass_CVmodel<-plsr(meta(ref.train.crs)$solubles_mass~as.matrix(ref.train.crs),
...
solubles_mass_valid <- which(!is.na(meta(ref.train.crs)$solubles_mass))
solubles_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[solubles_mass_valid],
                               Species=meta(ref.train.crs)$species[solubles_mass_valid],
                               Project=meta(ref.train.crs)$project[solubles_mass_valid],
                               measured=meta(ref.train.crs)$solubles_mass[solubles_mass_valid],
...
hemicellulose_mass_CVmodel<-plsr(meta(ref.train.crs)$hemicellulose_mass~as.matrix(ref.train.crs),
                                 ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_hemicellulose_mass_CVmodel <- selectNcomp(hemicellulose_mass_CVmodel, method = "onesigma", plot = FALSE)
hemicellulose_mass_valid <- which(!is.na(meta(ref.train.crs)$hemicellulose_mass))
hemicellulose_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[hemicellulose_mass_valid],
                                    Species=meta(ref.train.crs)$species[hemicellulose_mass_valid],
                                    Project=meta(ref.train.crs)$project[hemicellulose_mass_valid],
                                    measured=meta(ref.train.crs)$hemicellulose_mass[hemicellulose_mass_valid],
...
cellulose_mass_CVmodel<-plsr(meta(ref.train.crs)$cellulose_mass~as.matrix(ref.train.crs),
                             ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_cellulose_mass_CVmodel <- selectNcomp(cellulose_mass_CVmodel, method = "onesigma", plot = FALSE)
cellulose_mass_valid <- which(!is.na(meta(ref.train.crs)$cellulose_mass))
cellulose_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[cellulose_mass_valid],
                                Species=meta(ref.train.crs)$species[cellulose_mass_valid],
                                Project=meta(ref.train.crs)$project[cellulose_mass_valid],
                                measured=meta(ref.train.crs)$cellulose_mass[cellulose_mass_valid],
...
lignin_mass_CVmodel<-plsr(meta(ref.train.crs)$lignin_mass~as.matrix(ref.train.crs),
                          ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_lignin_mass_CVmodel <- selectNcomp(lignin_mass_CVmodel, method = "onesigma", plot = FALSE)
lignin_mass_valid <- which(!is.na(meta(ref.train.crs)$lignin_mass))
lignin_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[lignin_mass_valid],
                             Species=meta(ref.train.crs)$species[lignin_mass_valid],
                             Project=meta(ref.train.crs)$project[lignin_mass_valid],
                             measured=meta(ref.train.crs)$lignin_mass[lignin_mass_valid],
...
Cmass_CVmodel<-plsr(meta(ref.train.crs)$Cmass~as.matrix(ref.train.crs),
                    ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Cmass_CVmodel <- selectNcomp(Cmass_CVmodel, method = "onesigma", plot = FALSE)
Cmass_valid <- which(!is.na(meta(ref.train.crs)$Cmass))
Cmass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Cmass_valid],
                       Species=meta(ref.train.crs)$species[Cmass_valid],
                       Project=meta(ref.train.crs)$project[Cmass_valid],
                       measured=meta(ref.train.crs)$Cmass[Cmass_valid],
...
Nmass_CVmodel<-plsr(meta(ref.train.crs)$Nmass~as.matrix(ref.train.crs),
                    ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Nmass_CVmodel <- selectNcomp(Nmass_CVmodel, method = "onesigma", plot = FALSE)
Nmass_valid <- which(!is.na(meta(ref.train.crs)$Nmass))
Nmass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Nmass_valid],
                       Species=meta(ref.train.crs)$species[Nmass_valid],
                       Project=meta(ref.train.crs)$project[Nmass_valid],
                       measured=meta(ref.train.crs)$Nmass[Nmass_valid],
...
EWT_CVmodel<-plsr(meta(ref.train.crs)$EWT~as.matrix(ref.train.crs),
                  ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_EWT_CVmodel <- selectNcomp(EWT_CVmodel, method = "onesigma", plot = FALSE)
EWT_valid <- which(!is.na(meta(ref.train.crs)$EWT))
EWT_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[EWT_valid],
                     Species=meta(ref.train.crs)$species[EWT_valid],
                     Project=meta(ref.train.crs)$project[EWT_valid],
                     measured=meta(ref.train.crs)$EWT[EWT_valid],
...
LDMC_CVmodel<-plsr(meta(ref.train.crs)$LDMC~as.matrix(ref.train.crs),
                   ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_LDMC_CVmodel <- selectNcomp(LDMC_CVmodel, method = "onesigma", plot = FALSE)
LDMC_valid <- which(!is.na(meta(ref.train.crs)$LDMC))
LDMC_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[LDMC_valid],
                      Species=meta(ref.train.crs)$species[LDMC_valid],
                      Project=meta(ref.train.crs)$project[LDMC_valid],
                      measured=meta(ref.train.crs)$LDMC[LDMC_valid],
...
LMA_CVmodel<-plsr(meta(ref.train.crs)$LMA~as.matrix(ref.train.crs),
                  ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_LMA_CVmodel <- selectNcomp(LMA_CVmodel, method = "onesigma", plot = FALSE)
LMA_valid <- which(!is.na(meta(ref.train.crs)$LMA))
LMA_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[LMA_valid],
                     Species=meta(ref.train.crs)$species[LMA_valid],
                     Project=meta(ref.train.crs)$project[LMA_valid],
                     measured=meta(ref.train.crs)$LMA[LMA_valid],
...
chlA_mass_CVmodel<-plsr(meta(ref.train.crs)$chlA_mass~as.matrix(ref.train.crs),
                        ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_chlA_mass_CVmodel <- selectNcomp(chlA_mass_CVmodel, method = "onesigma", plot = FALSE)
chlA_mass_valid <- which(!is.na(meta(ref.train.crs)$chlA_mass))
chlA_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[chlA_mass_valid],
                           Species=meta(ref.train.crs)$species[chlA_mass_valid],
                           Project=meta(ref.train.crs)$project[chlA_mass_valid],
                           measured=meta(ref.train.crs)$chlA_mass[chlA_mass_valid],
...
chlB_mass_CVmodel<-plsr(meta(ref.train.crs)$chlB_mass~as.matrix(ref.train.crs),
                        ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_chlB_mass_CVmodel <- selectNcomp(chlB_mass_CVmodel, method = "onesigma", plot = FALSE)
chlB_mass_valid <- which(!is.na(meta(ref.train.crs)$chlB_mass))
chlB_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[chlB_mass_valid],
                           Species=meta(ref.train.crs)$species[chlB_mass_valid],
                           Project=meta(ref.train.crs)$project[chlB_mass_valid],
                           measured=meta(ref.train.crs)$chlB_mass[chlB_mass_valid],
...
car_mass_CVmodel<-plsr(meta(ref.train.crs)$car_mass~as.matrix(ref.train.crs),
                       ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_car_mass_CVmodel <- selectNcomp(car_mass_CVmodel, method = "onesigma", plot = FALSE)
car_mass_valid <- which(!is.na(meta(ref.train.crs)$car_mass))
car_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[car_mass_valid],
                          Species=meta(ref.train.crs)$species[car_mass_valid],
                          Project=meta(ref.train.crs)$project[car_mass_valid],
                          measured=meta(ref.train.crs)$car_mass[car_mass_valid],
...
Al_mass_CVmodel<-plsr(meta(ref.train.crs)$Al_mass~as.matrix(ref.train.crs),
                      ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Al_mass_CVmodel <- selectNcomp(Al_mass_CVmodel, method = "onesigma", plot = FALSE)
Al_mass_valid <- which(!is.na(meta(ref.train.crs)$Al_mass))
Al_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Al_mass_valid],
                         Species=meta(ref.train.crs)$species[Al_mass_valid],
                         Project=meta(ref.train.crs)$project[Al_mass_valid],
                         measured=meta(ref.train.crs)$Al_mass[Al_mass_valid],
...
Ca_mass_CVmodel<-plsr(meta(ref.train.crs)$Ca_mass~as.matrix(ref.train.crs),
                      ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Ca_mass_CVmodel <- selectNcomp(Ca_mass_CVmodel, method = "onesigma", plot = FALSE)
Ca_mass_valid <- which(!is.na(meta(ref.train.crs)$Ca_mass))
Ca_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Ca_mass_valid],
                         Species=meta(ref.train.crs)$species[Ca_mass_valid],
                         Project=meta(ref.train.crs)$project[Ca_mass_valid],
                         measured=meta(ref.train.crs)$Ca_mass[Ca_mass_valid],
...
Cu_mass_CVmodel<-plsr(meta(ref.train.crs)$Cu_mass~as.matrix(ref.train.crs),
                      ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Cu_mass_CVmodel <- selectNcomp(Cu_mass_CVmodel, method = "onesigma", plot = FALSE)
Cu_mass_valid <- which(!is.na(meta(ref.train.crs)$Cu_mass))
Cu_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Cu_mass_valid],
                         Species=meta(ref.train.crs)$species[Cu_mass_valid],
                         Project=meta(ref.train.crs)$project[Cu_mass_valid],
                         measured=meta(ref.train.crs)$Cu_mass[Cu_mass_valid],
...
Fe_mass_CVmodel<-plsr(meta(ref.train.crs)$Fe_mass~as.matrix(ref.train.crs),
                      ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Fe_mass_CVmodel <- selectNcomp(Fe_mass_CVmodel, method = "onesigma", plot = FALSE)
Fe_mass_valid <- which(!is.na(meta(ref.train.crs)$Fe_mass))
Fe_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Fe_mass_valid],
                         Species=meta(ref.train.crs)$species[Fe_mass_valid],
                         Project=meta(ref.train.crs)$project[Fe_mass_valid],
                         measured=meta(ref.train.crs)$Fe_mass[Fe_mass_valid],
...
K_mass_CVmodel<-plsr(meta(ref.train.crs)$K_mass~as.matrix(ref.train.crs),
                     ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_K_mass_CVmodel <- selectNcomp(K_mass_CVmodel, method = "onesigma", plot = FALSE)
K_mass_valid <- which(!is.na(meta(ref.train.crs)$K_mass))
K_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[K_mass_valid],
                        Species=meta(ref.train.crs)$species[K_mass_valid],
                        Project=meta(ref.train.crs)$project[K_mass_valid],
                        measured=meta(ref.train.crs)$K_mass[K_mass_valid],
...
Mg_mass_CVmodel<-plsr(meta(ref.train.crs)$Mg_mass~as.matrix(ref.train.crs),
                      ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Mg_mass_CVmodel <- selectNcomp(Mg_mass_CVmodel, method = "onesigma", plot = FALSE)
Mg_mass_valid <- which(!is.na(meta(ref.train.crs)$Mg_mass))
Mg_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Mg_mass_valid],
                         Species=meta(ref.train.crs)$species[Mg_mass_valid],
                         Project=meta(ref.train.crs)$project[Mg_mass_valid],
                         measured=meta(ref.train.crs)$Mg_mass[Mg_mass_valid],
...
Mn_mass_CVmodel<-plsr(meta(ref.train.crs)$Mn_mass~as.matrix(ref.train.crs),
                      ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Mn_mass_CVmodel <- selectNcomp(Mn_mass_CVmodel, method = "onesigma", plot = FALSE)
Mn_mass_valid <- which(!is.na(meta(ref.train.crs)$Mn_mass))
Mn_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Mn_mass_valid],
                         Species=meta(ref.train.crs)$species[Mn_mass_valid],
                         Project=meta(ref.train.crs)$project[Mn_mass_valid],
                         measured=meta(ref.train.crs)$Mn_mass[Mn_mass_valid],
...
Na_mass_CVmodel<-plsr(meta(ref.train.crs)$Na_mass~as.matrix(ref.train.crs),
                      ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Na_mass_CVmodel <- selectNcomp(Na_mass_CVmodel, method = "onesigma", plot = FALSE)
Na_mass_valid <- which(!is.na(meta(ref.train.crs)$Na_mass))
Na_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Na_mass_valid],
                         Species=meta(ref.train.crs)$species[Na_mass_valid],
                         Project=meta(ref.train.crs)$project[Na_mass_valid],
                         measured=meta(ref.train.crs)$Na_mass[Na_mass_valid],
...
P_mass_CVmodel<-plsr(meta(ref.train.crs)$P_mass~as.matrix(ref.train.crs),
                     ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_P_mass_CVmodel <- selectNcomp(P_mass_CVmodel, method = "onesigma", plot = FALSE)
P_mass_valid <- which(!is.na(meta(ref.train.crs)$P_mass))
P_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[P_mass_valid],
                        Species=meta(ref.train.crs)$species[P_mass_valid],
                        Project=meta(ref.train.crs)$project[P_mass_valid],
                        measured=meta(ref.train.crs)$P_mass[P_mass_valid],
...
Zn_mass_CVmodel<-plsr(meta(ref.train.crs)$Zn_mass~as.matrix(ref.train.crs),
                      ncomp=30,method = "oscorespls",validation="CV",segments=10)
ncomp_Zn_mass_CVmodel <- selectNcomp(Zn_mass_CVmodel, method = "onesigma", plot = FALSE)
Zn_mass_valid <- which(!is.na(meta(ref.train.crs)$Zn_mass))
Zn_mass_pred<-data.frame(ID=meta(ref.train.crs)$sample_id[Zn_mass_valid],
                         Species=meta(ref.train.crs)$species[Zn_mass_valid],
                         Project=meta(ref.train.crs)$project[Zn_mass_valid],
                         measured=meta(ref.train.crs)$Zn_mass[Zn_mass_valid],
...
for(i in 1:nreps){
  ...
  n.cal.spec<-nrow(ref.train.crs)
  train.jack<-sample(1:n.cal.spec,floor(0.7*n.cal.spec))
  test.jack<-setdiff(1:n.cal.spec,train.jack)
  
  calib.jack<-ref.train.crs[train.jack]
  val.jack<-ref.train.crs[test.jack]
...
solubles_mass.jack.pred<-apply.coefs(solubles_mass.jack.coefs,as.matrix(ref.test.crs))
...
solubles_mass.jack.df<-data.frame(pred.mean=solubles_mass.jack.stat[,1],
                                  ...
                                  Measured=meta(ref.test.crs)$solubles_mass,
                                  ncomp=ncomp_solubles_mass_CVmodel,
                                  Species=meta(ref.test.crs)$species,
                                  Project=meta(ref.test.crs)$project,
                                  functional.group=meta(ref.test.crs)$functional.group,
                                  ID=meta(ref.test.crs)$sample_id)
...
hemicellulose_mass.jack.pred<-apply.coefs(hemicellulose_mass.jack.coefs,as.matrix(ref.test.crs))
...
hemicellulose_mass.jack.df<-data.frame(pred.mean=hemicellulose_mass.jack.stat[,1],
                                       ...
                                       Measured=meta(ref.test.crs)$hemicellulose_mass,
...
cellulose_mass.jack.pred<-apply.coefs(cellulose_mass.jack.coefs,as.matrix(ref.test.crs))
...
cellulose_mass.jack.df<-data.frame(pred.mean=cellulose_mass.jack.stat[,1],
                                   ...
                                   Measured=meta(ref.test.crs)$cellulose_mass,
...
lignin_mass.jack.pred<-apply.coefs(lignin_mass.jack.coefs,as.matrix(ref.test.crs))
...
lignin_mass.jack.df<-data.frame(pred.mean=lignin_mass.jack.stat[,1],
                                ...
                                Measured=meta(ref.test.crs)$lignin_mass,
...
chlA_mass.jack.pred<-apply.coefs(chlA_mass.jack.coefs,as.matrix(ref.test.crs))
...
chlA_mass.jack.df<-data.frame(pred.mean=chlA_mass.jack.stat[,1],
                              ...
                              Measured=meta(ref.test.crs)$chlA_mass,
...
chlB_mass.jack.pred<-apply.coefs(chlB_mass.jack.coefs,as.matrix(ref.test.crs))
...
chlB_mass.jack.df<-data.frame(pred.mean=chlB_mass.jack.stat[,1],
                              ...
                              Measured=meta(ref.test.crs)$chlB_mass,
...
car_mass.jack.pred<-apply.coefs(car_mass.jack.coefs,as.matrix(ref.test.crs))
...
car_mass.jack.df<-data.frame(pred.mean=car_mass.jack.stat[,1],
                             ...
                             Measured=meta(ref.test.crs)$car_mass,
...
Cmass.jack.pred<-apply.coefs(Cmass.jack.coefs,as.matrix(ref.test.crs))
...
Cmass.jack.df<-data.frame(pred.mean=Cmass.jack.stat[,1],
                          ...
                          Measured=meta(ref.test.crs)$Cmass,
...
Nmass.jack.pred<-apply.coefs(Nmass.jack.coefs,as.matrix(ref.test.crs))
...
Nmass.jack.df<-data.frame(pred.mean=Nmass.jack.stat[,1],
                          ...
                          Measured=meta(ref.test.crs)$Nmass,
...
EWT.jack.pred<-apply.coefs(EWT.jack.coefs,as.matrix(ref.test.crs))
...
EWT.jack.df<-data.frame(pred.mean=EWT.jack.stat[,1],
                        ...
                        Measured=meta(ref.test.crs)$EWT,
...
LMA.jack.pred<-apply.coefs(LMA.jack.coefs,as.matrix(ref.test.crs))
...
LMA.jack.df<-data.frame(pred.mean=LMA.jack.stat[,1],
                        ...
                        Measured=meta(ref.test.crs)$LMA,
...
LDMC.jack.pred<-apply.coefs(LDMC.jack.coefs,as.matrix(ref.test.crs))
...
LDMC.jack.df<-data.frame(pred.mean=LDMC.jack.stat[,1],
                         ...
                         Measured=meta(ref.test.crs)$LDMC,
...
Al_mass.jack.pred<-apply.coefs(Al_mass.jack.coefs,as.matrix(ref.test.crs))
...
Al_mass.jack.df<-data.frame(pred.mean=Al_mass.jack.stat[,1],
                            ...
                            Measured=meta(ref.test.crs)$Al_mass,
...
Ca_mass.jack.pred<-apply.coefs(Ca_mass.jack.coefs,as.matrix(ref.test.crs))
...
Ca_mass.jack.df<-data.frame(pred.mean=Ca_mass.jack.stat[,1],
                            ...
                            Measured=meta(ref.test.crs)$Ca_mass,
...
Cu_mass.jack.pred<-apply.coefs(Cu_mass.jack.coefs,as.matrix(ref.test.crs))
...
Cu_mass.jack.df<-data.frame(pred.mean=Cu_mass.jack.stat[,1],
                            ...
                            Measured=meta(ref.test.crs)$Cu_mass,
...
Fe_mass.jack.pred<-apply.coefs(Fe_mass.jack.coefs,as.matrix(ref.test.crs))
...
Fe_mass.jack.df<-data.frame(pred.mean=Fe_mass.jack.stat[,1],
                            ...
                            Measured=meta(ref.test.crs)$Fe_mass,
...
K_mass.jack.pred<-apply.coefs(K_mass.jack.coefs,as.matrix(ref.test.crs))
...
K_mass.jack.df<-data.frame(pred.mean=K_mass.jack.stat[,1],
                           ...
                           Measured=meta(ref.test.crs)$K_mass,
...
Mg_mass.jack.pred<-apply.coefs(Mg_mass.jack.coefs,as.matrix(ref.test.crs))
...
Mg_mass.jack.df<-data.frame(pred.mean=Mg_mass.jack.stat[,1],
                            ...
                            Measured=meta(ref.test.crs)$Mg_mass,
...
Mn_mass.jack.pred<-apply.coefs(Mn_mass.jack.coefs,as.matrix(ref.test.crs))
...
Mn_mass.jack.df<-data.frame(pred.mean=Mn_mass.jack.stat[,1],
                            ...
                            Measured=meta(ref.test.crs)$Mn_mass,
...
Na_mass.jack.pred<-apply.coefs(Na_mass.jack.coefs,as.matrix(ref.test.crs))
...
Na_mass.jack.df<-data.frame(pred.mean=Na_mass.jack.stat[,1],
                            ...
                            Measured=meta(ref.test.crs)$Na_mass,
...
P_mass.jack.pred<-apply.coefs(P_mass.jack.coefs,as.matrix(ref.test.crs))
...
P_mass.jack.df<-data.frame(pred.mean=P_mass.jack.stat[,1],
                           ...
                           Measured=meta(ref.test.crs)$P_mass,
...
Zn_mass.jack.pred<-apply.coefs(Zn_mass.jack.coefs,as.matrix(ref.test.crs))
...
Zn_mass.jack.df<-data.frame(pred.mean=Zn_mass.jack.stat[,1],
                            ...
                            Measured=meta(ref.test.crs)$Zn_mass,
...
saveRDS(all.jack.coef.list,"SavedResults/all_jack_coefs_list_ref_crs.rds")
...
all.jack.df.list<-list(sol=solubles_mass.jack.df,
                       ...
                       P=P_mass.jack.df,
                       Zn=Zn_mass.jack.df)
saveRDS(all.jack.df.list,"SavedResults/all_jack_df_list_ref_crs.rds")
...
saveRDS(all.jack.stats.list,"SavedResults/all_jack_stats_list_ref_crs.rds")