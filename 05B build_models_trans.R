...
pdf("Images/VIP_trans.pdf",height=12,width=6,onefile = F)
...
saveRDS(all.jack.stats.list,"SavedResults/all_jack_stats_list_trans.rds")
...
trans_val_summary<-data.frame(variable=names(all.jack.df.list),
...
val_R2<-ggplot(R2.long,aes(y=value,x=variable))+
  geom_violin()+
  geom_point(data=trans_val_summary,
...
pdf("Images/violin_plots_trans.pdf",width=8,height=6,onefile=F)
